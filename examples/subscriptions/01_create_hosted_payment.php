...
/**
 * Create a hosted payment that includes a scheduled subscription agreement.
 *
 * Initializes a payment object with subscription details so the resulting payment can be used for recurring charges.
 *
...
// 2) Create hosted payment with scheduled subscription
$hostedPayment = $paymentApi->createHostedPayment(
    createHostedPaymentRequestForSubscription()
);
...
exampleWriteLine('1) Retrieve details: 02a_retrieve_subscription.php');
exampleWriteLine('2) Charge it now: 03_charge_subscription.php');
exampleWriteLine('3) Collect multiple payment IDs and run bulk charge/verify: 04_bulk_charge_subscriptions.php or 06_verify_subscriptions.php');
