 ...
 *
 * Retrieves charges associated with a bulk unscheduled charge operation by bulk ID.
 *
 * Endpoint documentation:
 * https://developer.nexigroup.com/nexi-checkout/en-EU/api/payment-v1/#v1-unscheduledsubscriptions-charges-bulkid-get
 */
...
// 2) Retrieve bulk unscheduled charges
$bulkStatusResult = $subscriptionApi->retrieveBulkUnscheduledCharges(
    $bulkId,
    $skip,
    $take,
    $pageNumber,
...
foreach ($bulkStatusResult->getPage() as $index => $charge) {
    ...
    exampleWriteLine('Unscheduled subscription ID: ' . $charge->getUnscheduledSubscriptionId());
    exampleWriteLine('Status: ' . $charge->getStatus()->value);
